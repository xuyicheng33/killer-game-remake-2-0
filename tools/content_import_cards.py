...
EFFECT_OPS = {"damage", "block", "apply_status"}
        ...
        if op == "damage":
            lines.extend(
                [
                    ...
                    f"\tdamage_effect_{suffix}.execute(targets)",
                ]
            )
        elif op == "block":
            lines.extend(
                [
                    ...
                    f"\tblock_effect_{suffix}.execute(targets)",
                ]
            )
        elif op == "apply_status":
            lines.extend(
                [
                    ...
                    f"\tstatus_effect_{suffix}.execute(targets)",
                ]
            )
...
def _generate_starting_deck_tres(deck_cards: list[str]) -> str:
    ...
    id_map: dict[str, str] = {}
    ext_lines = [
        "[ext_resource type=\"Script\" path=\"res://custom_resources/card_pile.gd\" id=\"1_card_pile_script\"]"
    ]
    ...
    lines.append(
        "cards = Array[Resource(\"res://custom_resources/card.gd\")]([%s])" % refs
    )
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    parser.add_argument(
        "--input",
        default="modules/content_pipeline/sources/cards/warrior_cards.json",
        help="input JSON file path (repo-relative or absolute)",
    )
    parser.add_argument(
        "--output-dir",
        default="characters/warrior/cards/generated",
        help="generated card output directory (repo-relative or absolute)",
    )
    parser.add_argument(
        "--deck-output",
        default="characters/warrior/warrior_starting_deck.tres",
        help="generated warrior starting deck resource",
    )
    parser.add_argument(
        "--report",
        default="modules/content_pipeline/reports/card_import_report.json",
