...
- `.git*`、`.editorconfig`
- `README.md`
- 主目录：`scenes`、`modules`、`global`、`art`、`characters`、`enemies`、`effects`、`custom_resources`、`docs`、`tools`、`references`

根目录禁止新增：

- 临时分析文档（统一放 `docs/session/`）
- 一次性脚本（统一放 `tools/` 或 `docs/archive/`）
...
- `modules -> scenes`
- `global -> scenes/modules` 的业务耦合
- UI 脚本直接写核心运行态（按 `tools/ui_shell_contract_check.sh` 执行）

## 4. 命名收口

- 存档主模块统一为 `modules/persistence`。
- `modules/seed_replay` 只作为占位，不新增业务实现。
...
- 允许独立外置或分仓；若移除，需同步调整 `tools/content_index.sh` 与相关文档。
