...
- `seed` 是整局随机入口的根字段，地图/敌方意图/奖励随机应接入统一 RNG 服务并由该种子驱动。
- 继续游戏场景要求恢复 RNG 流进度（不仅是恢复 `seed`），避免“读档后随机流重置”导致分歧。
- 同一 `seed` 在同一版本下应满足最小复现目标：
  - 前 3 层节点链路一致
  - 首战敌方行为链路一致（按当前可观测项）
- 最小复现日志应可定位一次 run，至少包含：`seed` / `floor` / `node` / `enemy`。
